 ...
 */
static int
setstr (HalDevice *d, char *buf, char *str, char *prop)
{
	if (strbegin (buf, str)) {
		char *value;
		...
		hal_device_property_set_string (d, prop, value);
...
static int
setstrsplit (HalDevice *d, char *buf, char *str, char *prop_a, char *prop_b)
{
	if (strbegin (buf, str)) {
		...
		*value_b = '\0';
		...
		hal_device_property_set_string (d, prop_a, value_a);
		hal_device_property_set_string (d, prop_b, value_b);
...
static void
parse_omap_hwinfo (HalDevice *d, char *buf)
{
	/* XXX: Think about this 'hardware' key -> 'system.hardware'? */
	setstr (d, buf, "product", "system.hardware.product");
	setstr (d, buf, "hw-build", "system.hardware.version");
	if (setstr (d, buf, "nolo", "system.firmware.version"))
		hal_device_property_set_string (d, "system.firmware.name", "NOLO");
}

static void
parse_omap_cpuinfo (HalDevice *d, char *buf)
{
	setstrsplit (d, buf, "Hardware",
		     "system.hardware.vendor", "system.firmware.product");
	setstr (d, buf, "Revision", "system.hardware.uuid");
	setstr (d, buf, "Serial", "system.hardware.serial");
}

static int
parse_omap_file (HalDevice *d, char *filename, void (*parse_fn)(HalDevice *d, char *))
{
	char buf[512];
	FILE *f;
...
static void
probe_omap (HalDevice *root)
{
	if (parse_omap_file (root, "/proc/component_version", parse_omap_hwinfo))
		return;
	if (parse_omap_file (root, "/proc/cpuinfo", parse_omap_cpuinfo))
		return;
}
#else
static void
