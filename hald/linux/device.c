 ...
 * Copyright (C) 2007, 2008, 2009 Nokia Corporation
 *
...
input_test_switch (HalDevice *d, const char *sysfs_path)
{
	...
	if (num_bits == 1 || num_bits == input_count_jack_switches (bitmask)) {
		...
		if (test_bit (SW_VIDEOOUT_INSERT, bitmask)) {
			is_jack_switch = TRUE;
			hal_device_add_capability (d, "input.jack.videoout");
		}
		if (test_bit (SW_JACK_PHYSICAL_INSERT, bitmask)) {
			/* This one is special, the insertion itself is going
			 * to be triggered with the button, so no need to
			 * additionally report the capability. */
			is_jack_switch = TRUE;
		}
...
mmc_host_add (const gchar *sysfs_path, const gchar *device_file, HalDevice *parent_dev, const gchar *parent_path)
{
	...
	slot_name = hal_util_get_string_from_file (sysfs_path, "slot_name");
	if (slot_name) {
		if (strncmp (slot_name, "slot:", strlen ("slot:")) == 0)
			slot_name += strlen ("slot:");

		if (strcmp (slot_name, "internal") != 0 &&
		    strcmp (slot_name, "external") != 0)
			slot_name = "unknown";

		hal_device_property_set_string (d, "mmc_host.slot_name", slot_name);
	}

	s = hal_util_get_string_from_file (sysfs_path, "/cover_switch");
	if (s) {
		hal_device_add_capability (d, "button");
		hal_device_property_set_string (d, "button.type", "cover");
		hal_device_property_set_bool (d, "button.has_state", FALSE);
	}
...
usb_add (const gchar *sysfs_path, const gchar *device_file, HalDevice *parent_dev, const gchar *parent_path)
{
	...
	if (strchr (bus_id, ':') == NULL) {
		...
		s = hal_util_get_string_from_file (sysfs_path, "../mode");
		if (s) {
			hal_device_add_capability (d, "button");
			hal_device_property_set_string (d, "button.type", "usb.cable");
			hal_device_property_set_bool (d, "button.has_state", FALSE);
		}
